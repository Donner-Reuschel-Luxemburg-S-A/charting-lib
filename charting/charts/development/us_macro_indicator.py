...
def main():
    ...
    philly_business_outlook_df, philly_business_outlook_title = blp.get_series(series_id="OUTFGAF Index", observation_start=start_time) #1967
    philly_nonmanu_df, philly_nonmanu_title = blp.get_series(series_id="PNMARADI Index", observation_start=start_time) #2011
    ...
    richmond_business_outlook_df, richmond_business_outlook_title = blp.get_series(series_id="RCSSCLBC Index", observation_start=start_time) #2011
    richmond_manu_df, richmond_manu_title = blp.get_series(series_id="RCHSINDX Index", observation_start=start_time) #1994
    ...
    df_macro_ind['y'] = (philly_business_outlook_df['y'] < 0).astype(int) + (ism_manu_df['y'] < 50).astype(int)+ (yc_df['y'] < 0).astype(int)+(credit_impulse_df['y']<0).astype(int) + (ism_serv_df['y'] < 50).astype(int)+(richmond_manu_df['y'] < 0).astype(int)

    df_macro_ind_long = philly_business_outlook_df * 0
    df_macro_ind_long['y'] = (philly_business_outlook_df['y'] < 0).astype(int) + (ism_manu_df['y'] < 50).astype(int)+ (yc_df['y'] < 0).astype(int)+(credit_impulse_df['y']<0).astype(int)+ (chicago_pmi_df['y'] < 50).astype(int)
