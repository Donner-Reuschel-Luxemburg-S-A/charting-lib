...
def main(**kwargs):
    ...
    chart = Chart(title=title, metadata=metadata, filename="eu_gdp_growth", num_y_axis=1)
    ...
    x = np.linspace(0, number_of_bars_per_x_label * (number_of_x_labels - 1) * (1 + margin) * width,
                    num=number_of_x_labels)
    multiplier = 0
