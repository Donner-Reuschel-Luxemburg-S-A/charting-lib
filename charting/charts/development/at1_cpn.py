...
def main(**kwargs):
   ...
   # metadata = Metadata(title=title, region=Region.EU, category=Category.INFLATION)
    chart = Chart(title=title, filename="at1_cpn",
                  language=kwargs.get('language', 'en'))
    chart.add_series(df['CPN'], df['YTM'], label='YTM/CPN', chart_type='scatter')
    ##
    fig = chart.fig
    ax = chart.axis[0]
    ...
    texts = []
    annotations = []
    for x, y, c in zip(df['CPN'],df['YTM'],  df.index):
    ...
    chart.configure_x_axis(label='CPN')
    chart.configure_y_axis(label='YTM')
