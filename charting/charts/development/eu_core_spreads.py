...
def main(**kwargs):
    ...
    common_index = pd.DatetimeIndex(set(df2.index).intersection(set(df3.index)).intersection(set(df1.index))).sort_values()
    title = "EU Core Spreads"
    metadata = Metadata(title=title, region=Region.EU, category=Category.RATES)
    chart = Chart(title=title, metadata=metadata, filename="eu_core_spreads.png")
    ...
    chart.add_series(row_index=0, x=common_index, y=(df2.loc[common_index, 'y']-df3.loc[common_index, 'y'])*100, label="KFW")
    chart.legend(4)
