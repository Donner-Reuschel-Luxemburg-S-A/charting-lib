...
def main(**kwargs):
    ...
    chart = Chart(title=title, filename="us_budget_balance_mom", metadata=metadata, language=kwargs.get('language', 'en'))
    ...
    chart = Chart(title=title, filename="us_budget_balance_yoy", metadata=metadata, language=kwargs.get('language', 'en'))
    chart.configure_x_axis(minor_locator=mdates.YearLocator(base=1), major_locator=mdates.YearLocator(base=5))
