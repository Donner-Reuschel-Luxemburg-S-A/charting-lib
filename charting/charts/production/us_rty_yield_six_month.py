...
def main():
    ...
    df1, t1 = blp.get_series(series_id='RTY Index', field="px_close_1d", observation_start=start.strftime("%Y%m%d"))

    title = "Russel 2000 - 6 Month Performance"

    metadata = Metadata(title=title, region=Region.US, category=Category.EQUITY)
    chart = Chart(title=title, metadata=metadata, filename="us_rty_yield_six_month.png")
