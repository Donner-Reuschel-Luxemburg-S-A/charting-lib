...
def main():
    ...
    title = f"Global Asset Class Performance - 1 Month ({start.strftime('%d.%m.%Y')} - {today.strftime('%d.%m.%Y')})"

    metadata = Metadata(title=title, region=Region.GLOBAL,
                        category=[Category.EQUITY, Category.FI, Category.ALTERNATIVES])

    chart = Chart(title=title, metadata=metadata, filename="global_asset_class_performance_one_month.png", num_rows=3)
