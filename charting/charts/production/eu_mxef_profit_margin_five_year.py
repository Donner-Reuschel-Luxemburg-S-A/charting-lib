...
def main():
    ...
    df1, t1 = blp.get_series(series_id='MXEF Index', field="RR836", observation_start=start.strftime("%Y%m%d"))

    title = "MSCI Emerging Markets Profit Margin"

    metadata = Metadata(title=title, region=Region.EM, category=Category.EQUITY)
    chart = Chart(title=title, metadata=metadata, filename="eu_mxef_profit_margin_five_year.png")

    chart.configure_y_axis(y_axis_index=0, label="%", minor_locator=MultipleLocator(0.25), major_locator=MultipleLocator(1))
