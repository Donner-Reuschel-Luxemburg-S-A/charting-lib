...
def main(**kwargs):
    ...
    df1, t1 = blp.get_series(series_id='RTY Index', field="px_close_1d", observation_start=observation_start.strftime("%Y%m%d"),
                             observation_end=observation_end.strftime("%Y%m%d"))

    title = "Russel 2000 Performance"

    metadata = Metadata(title=title, region=Region.US, category=Category.EQUITY)
    chart = Chart(title=title, metadata=metadata, filename="us_rty_yield.png")
