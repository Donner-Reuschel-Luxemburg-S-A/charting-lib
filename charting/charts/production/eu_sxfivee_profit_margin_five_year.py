...
def main():
    ...
    df1, t1 = blp.get_series(series_id='SX5E Index', field="RR836", observation_start=start.strftime("%Y%m%d"))

    title = "Euro Stoxx 50 Profit Margin"

    metadata = Metadata(title=title, region=Region.EU, category=Category.EQUITY)
    chart = Chart(title=title, metadata=metadata, filename="eu_sx5e_profit_margin_five_year.png")
