...
def main(**kwargs):
    ...
    stock_indices = ["SPX Index", "NDX Index", "RTY Index", "SXXP Index", "SX5E Index", "DAX Index", "NKY Index",
                     "HSI Index"]
    stock_dfs = [blp.get_series(series_id=idx, observation_start=start.strftime("%Y%m%d"),
                                observation_end=end.strftime("%Y%m%d")) for idx in stock_indices]
    ...
    cmdty_dfs = [blp.get_series(series_id=idx, observation_start=start.strftime("%Y%m%d"),
                                observation_end=end.strftime("%Y%m%d")) for idx in
                 cmdty_indices]
    ...
    fi_dfs = [blp.get_series(series_id=idx, observation_start=start.strftime("%Y%m%d"),
                             observation_end=end.strftime("%Y%m%d")) for idx in
              fi_indices]
