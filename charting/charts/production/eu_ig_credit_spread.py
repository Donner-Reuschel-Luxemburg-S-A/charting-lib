...
def main(**kwargs):
    ...
    chart.add_series(row_index=0, x=df1.index, y=mean_val, label="Corporate A - 10Y Avg", linestyle="--", color=style.get_color(0))

    mean_val = [df2['y'].mean()] * len(df2.index)
    chart.add_series(row_index=1, x=df2.index, y=df2['y'], label="Corporate BBB", color=style.get_color(1))
    chart.add_series(row_index=1, x=df2.index, y=mean_val, label="Corporate BBB - 10Y Avg", linestyle="--", color=style.get_color(1))
