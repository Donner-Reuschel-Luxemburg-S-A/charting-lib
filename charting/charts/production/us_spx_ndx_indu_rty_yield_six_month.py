...
def main():
    ...
    df1, t1 = blp.get_series(series_id='SPX Index', field="px_close_1d", observation_start=start.strftime("%Y%m%d"))
    df2, t2 = blp.get_series(series_id='NDX Index', field="px_close_1d", observation_start=start.strftime("%Y%m%d"))
    df3, t3 = blp.get_series(series_id='INDU Index', field="px_close_1d", observation_start=start.strftime("%Y%m%d"))
    df4, t4 = blp.get_series(series_id='RTY Index', field="px_close_1d", observation_start=start.strftime("%Y%m%d"))

    title = "S&P 500, Nasdaq 100, Dow Jones & Russel 2000 - 6 Month Performance"

    metadata = Metadata(title=title, region=Region.US, category=Category.EQUITY)
    chart = Chart(title=title, metadata=metadata, filename="us_spx_ndx_indu_rty_yield_six_month.png")
