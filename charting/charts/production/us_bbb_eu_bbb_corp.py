...
def main(**kwargs):
    ...
    blp = BloombergSource()
    df, t = blp.get_series(series_id='LCB1TRUU Index', observation_start=observation_start.strftime("%Y%m%d"),
    ...
    df2, t2 = blp.get_series(series_id='I02561EU Index', observation_start=observation_start.strftime("%Y%m%d"),
                             observation_end=observation_end.strftime("%Y%m%d"), field='BX218')
    title = 'USD BBB - EUR BBB Spread'
    t = 'Bloomberg Baa Corporate Total Return Index Value Unhedged USD'
    ...
    chart = Chart(title=title, num_rows=2, filename="us_bbb_eu_bbb_corp", metadata=metadata, language=kwargs.get('language', 'en'))
