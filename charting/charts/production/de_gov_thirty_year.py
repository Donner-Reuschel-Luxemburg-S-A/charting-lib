...
def main(**kwargs):
    ...
    blp = BloombergSource()
    df1, t1 = blp.get_series(series_id='GDBR30 Index', observation_start=observation_start.strftime("%Y%m%d"),
                             observation_end=observation_end.strftime("%Y%m%d"))

    title = "Germany Government Bonds 30-Year"
    metadata = Metadata(title=title, region=Region.DE, category=Category.RATES)
    chart = Chart(title=title, metadata=metadata, filename="de_gov_30y", language=kwargs.get('language', 'en'))
