...
def main(**kwargs):
    ...
    df2, t2 = blp.get_series(series_id='I02003EU Index', observation_start=observation_start.strftime("%Y%m%d"),
    ...
    chart.add_series(row_index=1, x=df4.index, y=df4['y'], label="Government Related", color=style.get_color(1))
    chart.add_series(row_index=1, x=df4.index, y=mean_val, label="Government Related - 10Y Avg", linestyle="--", color=style.get_color(1))
